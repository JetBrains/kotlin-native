...
private fun primitives() {
    ...
    assertEquals(genericVectorOf(2f, 4f, 5f, 8f), PrimitiveTestSubject.simdFn())
}
...
private fun aggregates() {
    ...
    AggregateTestSubject.simd_quatfFn().useContents {
        assertEquals(genericVectorOf(1f, 4f, 9f, 25f), vector)
    }
    AggregateTestSubject.geterogeneousSmallFn().useContents {
        assertEquals(1, s1)
        assertEquals(genericVectorOf(1f, 4f, 9f, 25f), v2)
        assertEquals(3f, f3)
        assertEquals(4, i4)
    }
}
