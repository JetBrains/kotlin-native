...
@SharedImmutable
val global1: FreezableWorkerBoundReference<A> = FreezableWorkerBoundReference(A(3))
...
@SharedImmutable
val global2: FreezableWorkerBoundReference<A> = FreezableWorkerBoundReference(A(3))
...
@SharedImmutable
val global3: FreezableWorkerBoundReference<A> = FreezableWorkerBoundReference(A(3).freeze())
...
@SharedImmutable
val global4: FreezableWorkerBoundReference<A> = FreezableWorkerBoundReference(A(3))
...
@SharedImmutable
val global5: FreezableWorkerBoundReference<A> = FreezableWorkerBoundReference(A(3))
...
@SharedImmutable
val global6: FreezableWorkerBoundReference<A> = FreezableWorkerBoundReference(A(3))
...
@SharedImmutable
val global7: FreezableWorkerBoundReference<A> = FreezableWorkerBoundReference(A(3))
...
fun testLocalAccessOnWorkerFrozenInitiallyFrozen() {
    val local = FreezableWorkerBoundReference(A(3).freeze()).freeze()
    assertEquals(3, local.value.a)
    assertEquals(3, local.valueOrNull?.a)
...
fun testLocalDenyAccessOnMainThreadFrozen() {
    val worker = Worker.start()
    val future = worker.execute(TransferMode.SAFE, {}) {
        FreezableWorkerBoundReference(A(3)).freeze()
    }
...
fun getOwnerAndWeaksFrozen(initial: Int): Triple<AtomicReference<FreezableWorkerBoundReference<A>?>, WeakReference<FreezableWorkerBoundReference<A>>, WeakReference<A>> {
    val ref = FreezableWorkerBoundReference(A(initial)).freeze()
    val refOwner: AtomicReference<FreezableWorkerBoundReference<A>?> = AtomicReference(ref)
...
fun doNotCollectInWorkerFrozen(worker: Worker, semaphore: AtomicInt): Future<FreezableWorkerBoundReference<A>> {
    val ref = FreezableWorkerBoundReference(A(3)).freeze()
...
class B1 {
    lateinit var b2: FreezableWorkerBoundReference<B2>
}
...
fun createCyclicGarbageFrozen(): Triple<AtomicReference<FreezableWorkerBoundReference<B1>?>, WeakReference<B1>, WeakReference<B2>> {
    ...
    val ref1Owner: AtomicReference<FreezableWorkerBoundReference<B1>?> = AtomicReference(ref1)
    val ref1Weak = WeakReference(ref1.value)

    val ref2 = FreezableWorkerBoundReference(B2(ref1)).freeze()
...
fun doesNotCollectCyclicGarbageFrozen() {
    ...
    // If these asserts fail, that means FreezableWorkerBoundReference managed to clean up cyclic garbage all by itself.
...
fun createCrossThreadCyclicGarbageFrozen(
        worker: Worker
): Triple<AtomicReference<FreezableWorkerBoundReference<B1>?>, WeakReference<B1>, WeakReference<B2>> {
    val ref1 = FreezableWorkerBoundReference(B1()).freeze()
    val ref1Owner: AtomicReference<FreezableWorkerBoundReference<B1>?> = AtomicReference(ref1)
    val ref1Weak = WeakReference(ref1.value)
...
class C1 {
    lateinit var c2: AtomicReference<FreezableWorkerBoundReference<C2>?>
...
data class C2(val c1: AtomicReference<FreezableWorkerBoundReference<C1>>)

fun createCyclicGarbageWithAtomicsFrozen(): Triple<AtomicReference<FreezableWorkerBoundReference<C1>?>, WeakReference<C1>, WeakReference<C2>> {
    val ref1 = FreezableWorkerBoundReference(C1()).freeze()
    val ref1Weak = WeakReference(ref1.value)

    val ref2 = FreezableWorkerBoundReference(C2(AtomicReference(ref1))).freeze()
    val ref2Weak = WeakReference(ref2.value)
...
fun dispose(refOwner: AtomicReference<FreezableWorkerBoundReference<C1>?>) {
...
fun doesNotCollectCyclicGarbageWithAtomicsFrozen() {
    ...
    // If these asserts fail, that means AtomicReference<FreezableWorkerBoundReference> managed to clean up cyclic garbage all by itself.
    assertNotNull(ref1Weak.value)
    assertNotNull(ref2Weak.value)
}
...
fun createCrossThreadCyclicGarbageWithAtomicsFrozen(
        worker: Worker
): Triple<AtomicReference<FreezableWorkerBoundReference<C1>?>, WeakReference<C1>, WeakReference<C2>> {
    val ref1 = FreezableWorkerBoundReference(C1()).freeze()
    val ref1Weak = WeakReference(ref1.value)
...
fun testExceptionMessageFrozen() {
    ...
    val messagePattern = Regex("illegal attempt to access non-shared runtime\\.concurrent\\.worker_bound_reference1\\.A@[a-f0-9]+ bound to `$ownerName` from `${Worker.current.name}`")
