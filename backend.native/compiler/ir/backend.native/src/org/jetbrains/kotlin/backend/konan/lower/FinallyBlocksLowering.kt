...
internal class FinallyBlocksLowering(val context: Context): FileLoweringPass, IrElementTransformerVoidWithContext() {
    ...
    private data class Break(val loop: IrLoop): HighLevelJump {
        override fun toIr(context: Context, startOffset: Int, endOffset: Int, value: IrExpression)
                = IrBlockImpl(startOffset, endOffset, context.ir.symbols.nothing.defaultType, null,
                statements = listOf(
                        value,
                        IrBreakImpl(startOffset, endOffset, context.ir.symbols.nothing.defaultType, loop)
                ))
    }
    ...
    private data class Continue(val loop: IrLoop): HighLevelJump {
        override fun toIr(context: Context, startOffset: Int, endOffset: Int, value: IrExpression)
                ...
                statements = listOf(
                        value,
                        IrContinueImpl(startOffset, endOffset, context.ir.symbols.nothing.defaultType, loop)
                ))
    }
    ...
    override fun visitTry(aTry: IrTry): IrExpression {
        ...
        irBuilder.run {
            ...
            val catchParameter = IrVariableImpl(
                    startOffset, endOffset, IrDeclarationOrigin.CATCH_PARAMETER, parameter, this@FinallyBlocksLowering.context.ir.symbols.throwable.defaultType)
    ...
    private val nothingType: IrType = this.context.irBuiltIns.nothingType

    fun IrBuilderWithScope.irReturn(target: IrReturnTargetSymbol, value: IrExpression) =
            IrReturnImpl(startOffset, endOffset, nothingType, target, value)
