...
internal class TypeParameterEliminator(
        val context: Context,
        val oldFunction: IrFunction,
        val newFunction: IrFunction,
        val typeParameter: IrTypeParameter,
        val type: IrType): IrBuildingTransformer(context) {

    private var functionDeclarationStack = stackOf<IrFunction>()
    private val variables = mutableMapOf<VariableDescriptor, IrVariable>()

    init {
        functionDeclarationStack.push(oldFunction)
    }
    ...
    override fun visitReturn(expression: IrReturn): IrExpression {
        return context.createIrBuilder(newFunction.symbol).run {
    ...
    override fun visitGetValue(expression: IrGetValue): IrExpression {
        ...
        return context.createIrBuilder(symbol).run {
            when (descriptor) {
                is ValueParameterDescriptor -> irGet(newFunction.getIrValueParameter(descriptor))
