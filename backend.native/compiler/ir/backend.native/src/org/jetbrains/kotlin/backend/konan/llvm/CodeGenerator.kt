...
internal class FunctionGenerationContext(val function: LLVMValueRef,
    ...
    fun loadSlot(address: LLVMValueRef, isVar: Boolean, resultLifetime: Lifetime = Lifetime.IRRELEVANT, name: String = ""): LLVMValueRef {
        ...
        if (isObjectRef(value) && isVar) {
            // TODO: Is it correct?
            val slot = lifetimeToSlot(if (resultLifetime == Lifetime.RETURN_VALUE) resultLifetime else Lifetime.LOCAL(0))
            storeAny(value, slot)
        }
        return value
    }
...
//                }
    ...
    fun catchKotlinException(): LLVMValueRef {
        ...
        val exceptionPtr = loadSlot(exceptionPtrPtr, true, Lifetime.IRRELEVANT, "exception")
