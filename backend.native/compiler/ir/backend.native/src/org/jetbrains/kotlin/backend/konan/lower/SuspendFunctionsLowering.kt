...
import org.jetbrains.kotlin.backend.konan.irasdescriptors.isUnit
...
internal class SuspendFunctionsLowering(val context: Context): DeclarationContainerLoweringPass {
                ...
                +irReturn(
                        irCall(coroutine.doResumeFunction.symbol).apply {
                            dispatchReceiver = irCall(coroutine.coroutineConstructor.symbol).apply {
                                val functionParameters = irFunction.explicitIrParameters
                                ...
                                putValueArgument(functionParameters.size,
                                        irCall(getContinuationSymbol.owner, listOf(irFunction.returnType)))
                            }
    ...
    private inner class CoroutineBuilder(val irFunction: IrFunction, val functionReference: IrFunctionReference?) {
        ...
        private val functionParameters1 = irFunction.explicitIrParameters
        ...
        private val unboundFunctionParameters = boundFunctionParameters?.let { functionParameters1 - it }
        ...
        private val create1Function = coroutineImplSymbol.owner.simpleFunctions().single { it.name.asString() == "create" && it.valueParameters.size == 1 }
        private val create1FunctionDescriptor = create1Function.descriptor
        ...
        fun build(): BuiltCoroutine {
            ...
            if (unboundFunctionParameters != null) {
                ...
                val unboundParameterTypes = unboundFunctionParameters.map { it.type.toKotlinType() }
            ...
            val coroutineConstructorBuilder = createConstructorBuilder().also { it.initialize() }

            val doResumeFunction = coroutineImplSymbol.owner.simpleFunctions().single { it.name.asString() == "doResume" }
            val doResumeFunctionDescriptor = doResumeFunction.descriptor
            val doResumeMethodBuilder = createDoResumeMethodBuilder(doResumeFunction, coroutineClass).also {
                it.initialize()
            }
            ...
            if (functionReference != null) {
                ...
                createMethodBuilder = createCreateMethodBuilder(
                        ...
                        coroutineClass                 = coroutineClass).also { it.initialize() }
        ...
        private fun createConstructorBuilder()
            ...
            override fun buildIr(): IrConstructor {
                ...
                argumentToPropertiesMap = functionParameters1.associate {
                    it.descriptor to buildPropertyWithBackingField(it.name, it.type, false)
                }
                ...
                return IrConstructorImpl(
                    ...
                    returnType  = coroutineClass.symbol.defaultType

                    (irFunction.explicitIrParameters
                                    + coroutineImplConstructorSymbol.owner.valueParameters[0] // completion.
                            ).mapIndexedTo(this.valueParameters) { index, parameter -> parameter.copyAsValueParameter(descriptor.valueParameters[index]) }
                    ...
                    body = irBuilder.irBlockBody {
                        ...
                        functionParameters.forEachIndexed { index, parameter ->
                            +irSetField(irGet(coroutineClassThis), argumentToPropertiesMap[parameter]!!, irGet(valueParameters[index]))
                        }
                    }
                }
            }
        ...
        private fun createFactoryConstructorBuilder(boundParams: List<IrValueParameter>)
            ...
            override fun buildIr(): IrConstructor {
                ...
                return IrConstructorImpl(
                    ...
                    boundParams.mapIndexedTo(this.valueParameters) { index, parameter ->
                        parameter.copyAsValueParameter(descriptor.valueParameters[index])
                    }
                    ...
                    body = irBuilder.irBlockBody {
                        ...
                        boundParams.forEachIndexed { index, parameter ->
                            +irSetField(irGet(coroutineClassThis), argumentToPropertiesMap[parameter.descriptor]!!, irGet(valueParameters[index]))
                        }
                    }
                }
            }
        ...
        private fun createCreateMethodBuilder(unboundArgs: List<IrValueParameter>,
            ...
            override fun buildIr(): IrSimpleFunction {
                ...
                return IrFunctionImpl(
                    ...
                    this.createDispatchReceiverParameter(coroutineClass)

                    (unboundArgs + create1CompletionParameter).mapIndexedTo(this.valueParameters) { index, parameter ->
                        parameter.copyAsValueParameter(descriptor.valueParameters[index])
                    }
                    ...
                    body = irBuilder.irBlockBody(startOffset, endOffset) {
                        +irReturn(
                                irCall(coroutineConstructor).apply {
                                    var unboundIndex = 0
                                    val unboundArgsSet = unboundArgs.map { it.descriptor }.toSet()
        ...
        private fun createInvokeMethodBuilder(suspendFunctionInvokeFunctionDescriptor: FunctionDescriptor,
            ...
            override fun doInitialize() {
                val descriptor = symbol.descriptor as SimpleFunctionDescriptorImpl
                val valueParameters = createFunction.descriptor.valueParameters
                        // Skip completion - invoke() already has it implicitly as a suspend function.
                        .take(createFunction.descriptor.valueParameters.size - 1)
            ...
            override fun buildIr(): IrSimpleFunction {
                ...
                return IrFunctionImpl(
                    ...
                    createFunction.valueParameters
                            ...
                            .mapTo(this.valueParameters) { it.copyAsValueParameter(descriptor.valueParameters[it.index]) }
                                ...
                                irCall(doResumeFunction).apply {
                                    dispatchReceiver = irCall(createFunction).apply {
                                        ...
                                        putValueArgument(valueParameters.size,
                                                irCall(getContinuationSymbol, listOf(returnType)))
                                    }
                                    putValueArgument(0, irGetObject(symbols.unit))       // value
                                    putValueArgument(1, irNull())       // exception
                                }
        ...
        private fun buildPropertyWithBackingField(name: Name, type: IrType, isMutable: Boolean): IrField = createField(
        ...
        private fun createDoResumeMethodBuilder(doResumeFunction: IrFunction, coroutineClass: IrClass)
            ...
            private val doResumeFunctionDescriptor get() = doResumeFunction.descriptor
            ...
            override fun buildIr(): IrSimpleFunction {
                ...
                val function = IrFunctionImpl(
                    ...
                    doResumeFunction.valueParameters.mapIndexedTo(this.valueParameters) { index, it ->
                        it.copyAsValueParameter(descriptor.valueParameters[index])
                    }
                ...
                val label = coroutineImplSymbol.owner.declarations.filterIsInstance<IrProperty>().single { it.name.asString() == "label" }
                    ...
                    liveLocals.values.forEach { scope ->
                        scope.forEach {
                            localToPropertyMap.getOrPut(it.symbol) {
                                buildPropertyWithBackingField(it.descriptor.name, it.type, true)
                            }
                        }
                    }
        ...
        private val saveState = IrFunctionImpl(UNDEFINED_OFFSET, UNDEFINED_OFFSET, IrDeclarationOrigin.DEFINED, SimpleFunctionDescriptorImpl.create(
        ...
        private val saveStateSymbol = saveState.symbol

        private val restoreState = IrFunctionImpl(UNDEFINED_OFFSET, UNDEFINED_OFFSET, IrDeclarationOrigin.DEFINED, SimpleFunctionDescriptorImpl.create(
                irFunction.descriptor,
                Annotations.EMPTY,
        ...
        private val restoreStateSymbol = restoreState.symbol
                ...
                irBuilder.run {
                    ...
                    for ((index, child) in children.withIndex()) {
                        ...
                        if (transformedChild.isPure() || !hasSuspendCallInTail[index])
                            ...
                            val tmp = scope.createTemporaryVariable(transformedChild)
                    ...
                    val expressionResult = when {
                        suspendCall.type.isUnit() -> irImplicitCoercionToUnit(suspensionPoint)
                            ...
                            IrTypeOperatorCallImpl(startOffset, endOffset, suspendCall.type, IrTypeOperator.CAST, suspendCall.type, suspendCall.type.classifierOrFail, suspensionPoint)
                        }
                    }
                    return irBlock(expression) {
