...
internal class ClassLayoutBuilder(val irClass: IrClass, val context: Context, val isLowered: Boolean) {
    ...
    fun vtableIndex(function: IrSimpleFunction): Int {
        ...
        if (index < 0) {
            println("VTABLE for CLASS ${irClass.render()}")
            vtableEntries.forEach {
                println("${it.function} ${it.function.render()}")
            }
            throw Error(function.render() + " $function " + " (${function.symbol.descriptor}) not in vtable of " + irClass.render())
        }
        return index
    }
