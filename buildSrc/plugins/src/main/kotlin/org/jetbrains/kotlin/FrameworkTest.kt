 ...
 * with konanArtifacts { framework(frameworkName, targets: [ testTarget] ) }
 *
...
open class FrameworkTest : DefaultTask() {
    ...
    fun run() {
        ...
        // Sign framework
        val signResult = runProcess(executor = localExecutor, executable = "/usr/bin/codesign",
                args = listOf("--verbose", "-s", "-", Paths.get(frameworkPath, "$frameworkName.framework").toString()))
        check(signResult.exitCode == 0, { "Codesign failed with: ${signResult.stdOut + signResult.stdErr}" })
    ...
    private fun swiftc(sources: List<String>, options: List<String>, output: Path) {
        ...
        println("""$compiler finished with exit code: $exitCode
