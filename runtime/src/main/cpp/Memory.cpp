...
void leaveFrame(ObjHeader** start, int parameters, int count) {
  ...
  if (Strict) {
    ...
    RuntimeAssert(false, "Free frame elements");
  }
}
...
OBJ_GETTER(ReadHeapRefNoLock, ObjHeader* object, KInt index) {
   // TODO: MMM
  RETURN_RESULT_OF(readHeapRefNoLock<true>, object, index);
}
...
OBJ_GETTER(DerefStablePointer, KNativePtr pointer) {
  // TODO: MMM
  RETURN_RESULT_OF(derefStablePointer<true>, pointer);
}
