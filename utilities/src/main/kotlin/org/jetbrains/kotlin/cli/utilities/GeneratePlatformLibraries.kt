...
private fun generateLibrary(
    ...
    val buildDirectory = tmpDirectory.child("build-${def.name}-${Random.nextLong()}").apply { mkdirs() }
    val tmpKlib = buildDirectory.child(def.name)
    ...
    sorted.forEach { def ->
        executorPool.execute {
            ...
            try {
                ...
                generateLibrary(target, def, inputDirectory, outputDirectory, tmpDirectory, logger, saveTemps)
                if (cacheDirectory != null) {
