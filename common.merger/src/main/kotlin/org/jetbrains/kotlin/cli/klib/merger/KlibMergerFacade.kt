...
class KlibMergerFacade(private val repository: File, private val hostManager: PlatformManager) {
    ...
    fun merge(libs: List<KonanLibrary>): LinkData {
        val modulesToMerge = loadDescriptors(repository, libs)
        val targets = libs.map { lib -> lib.targetList.map { hostManager.targetByName(it) } }

        val map = (modulesToMerge zip targets).map { (module, targets) -> ModuleWithTargets(module, targets) }
        val mergedModules = DeclarationDescriptorMerger(LockBasedStorageManager(), DefaultBuiltIns.Instance).merge(map)
    ...
    fun diff(libs: List<KonanLibrary>): List<ModuleWithTargets> {
        ...
        return DeclarationDescriptorDiffer(LockBasedStorageManager(), DefaultBuiltIns.Instance).diff(map)
    }
